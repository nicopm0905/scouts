...
import { ArrowLeft, Coins, Euro, Edit, Trash2, Tent, Users, FileText, UserPlus, MessageCircle, FileDown, Copy, X, FolderArchive, CheckCircle2, AlertTriangle, Circle, ChevronRight, Box } from 'lucide-vue-next'
...
const activitiesByDay = computed(() => {
    if (!props.event.activities) return {}
    const grouped = {}
    props.event.activities.forEach(act => {
...
<template>
    ...
    <AppLayout>
        ...
        <section v-if="preparation" class="mb-6 overflow-hidden rounded-xl border border-ink-200 bg-white shadow-card">
            <div class="flex flex-col gap-2 border-b border-ink-100 bg-slate-50/70 p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="space-y-6 lg:col-span-2">
                ...
                <section v-if="event.activities && event.activities.length > 0" class="card-pad">
                    <h2 class="section-title mb-4">Cronograma del Evento (MSC)</h2>
                    <div class="space-y-6 border-l-2 border-slate-100 pl-4">
                            ...
                            <div class="relative">
                                ...
                                <div class="space-y-4">
                                    <template v-for="(slotActivities, slot) in groupBySlot(dayActivities)" :key="slot">
                                        <div class="pl-4">
                                            <h4 class="mb-2 text-xs font-semibold text-slate-500">{{ slot || 'Sin franja horaria' }}</h4>
                                            <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-2">
                                                <Link 
                                                    v-for="act in slotActivities" 
                                                    :key="act.id" 
                                                    :href="route('activities.show', act.id)"
                                                    class="group block rounded-lg border border-slate-200 bg-white p-3 shadow-sm hover:border-brand-300 hover:shadow-md transition"
                                                >
                                                    <div class="flex items-start justify-between gap-2">
                                                        <span class="text-xs font-bold text-slate-400 group-hover:text-brand-500">
                                                            Actividad {{ act.activity_number || '-' }}
                                                        </span>
                                                        <span class="inline-flex rounded-md bg-slate-50 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500">
                                                            {{ act.duration_minutes }} min
                                                        </span>
                                                    </div>
                                                    <p class="mt-1 text-sm font-semibold text-slate-700">{{ act.title }}</p>
                                                </Link>
                                            </div>
                                        </div>
                                    </template>
